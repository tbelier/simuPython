...
class ROV:
    ...
    def __init__(self, eta0, nu0, nup0):
        ...
        self.T200 = T200("/home/tbelier/Documents/GIT/python-simu/ROV/thruster16V.csv")
    ...
    def get_C(self):
        ...
        C_RB=np.array([[0,0,0,0,self.m*w,-self.m*v],
        ...
        C_A=np.array([[0,0,0,0,-self.Zwp*w,self.Yvp*v],
    ...
    def control(self, tau_bar):
        print(f"tau_bar*182/100 : {tau_bar*182/100}")
        Fx_bar,Fy_bar,Fz_bar,Tx_bar,Ty_bar,Tz_bar = tau_bar.flatten()*182/100
        Tplus = np.linalg.pinv(self.T) # Tplus = T.T@inv(T@T.T)
        u_f = Tplus@tau_bar#np.array([[Fx_bar],[Fy_bar],[Fz_bar],[Tx_bar],[Ty_bar],[Tz_bar]])
        #print(f"u_f : {u_f}")
        return u_f
    ...
    def get_tau(self,u):
        ...
        prend en entrée u = [[u0], et renvoie en sortie tau = [[Fx],
                             ...
                             [u7]]                             
        """
        u_f = [[self.T200.get_force(u[k,0])] for k in range(0,len(u))]
        #print(f"u_f : {u_f}")
        tau = self.T@u_f
        #print(f"tau : {tau}")
        return tau

    def f(self,tau_bar):#u): #fonction qui permet de faire évoluer le système !
        """
        ...
        etapp = J@(inv(M)@(tau_bar-C@nu-D@nu-g)-J_1_p@etap)
